...
static void writeWavHeader(File& f, uint32_t sr, uint16_t bits, uint16_t ch) {
  ...
  (void)f.write(h, 44);
  f.flush();
  f.seek(total);  // ★書き位置を「元の末尾」に戻す（必須）
}
...
RecResult recordingAutoSegmented(uint32_t totalSeconds,
                                 uint32_t segmentSeconds,
                                 const SessionConfig* sessionOpt,
                                 const AgcConfig* agcOpt) {
  SessionConfig s = getDefaultSession();
  AgcConfig a = getDefaultAgc();
  if (sessionOpt) s = *sessionOpt;
  if (agcOpt) a = *agcOpt;

  const uint16_t bytesPerSample = s.bitsPerSamp / 8;
  const uint32_t bytesPerSec = s.sampleRate * s.channels * bytesPerSample;

  // サイズ基準（秒→バイト）だけで切替を判断。タイマでは絶対に切り替えない
  const uint32_t segBytesTarget = (segmentSeconds ? segmentSeconds : 60) * bytesPerSec;
  const uint64_t totalBytesMax = (totalSeconds ? (uint64_t)totalSeconds * bytesPerSec : UINT64_MAX);

  // バッファ
  int16_t* bufPtr = nullptr;
  std::unique_ptr<int16_t[]> bufHolder;
  if (s.extBuffer && s.extBufSamps >= s.blockSamples) bufPtr = s.extBuffer;
  else {
    bufHolder.reset(new int16_t[s.blockSamples]);
    bufPtr = bufHolder.get();
  }
  ...
  // 状態
  dcReset();
  float agc_lin_gain = 1.0f;
  const float dcAlpha = dc_alpha_for(s.sampleRate);

  // ファイルopenヘルパ
  auto open_new_file = [&](File& f, String& path) -> bool {
    path = nextWavPath(s.dir);  // 連番生成:contentReference[oaicite:2]{index=2}
    ...
    f.flush();  // 44B確実化:contentReference[oaicite:3]{index=3}
    return true;
  };

  File f;
  String curPath;
  if (!open_new_file(f, curPath)) return RecResult::FileOpenError;

  uint32_t segWritten = 0;
  uint64_t totalWritten = 0;

  // 初回だけ dropHeadMs を適用。以降のセグメントでは適用しない
  uint32_t dropBytes = (s.dropHeadMs * s.sampleRate / 1000) * s.channels * bytesPerSample;

  // ヘッダ“上書き”とflushは定期的に行うが、絶対にcloseしない
  const uint32_t FLUSH_INTERVAL_MS = 100;
  const uint32_t HDR_INTERVAL_MS = 500;
  uint32_t nextFlushAt = millis() + FLUSH_INTERVAL_MS;
  uint32_t nextHdrAt = millis() + HDR_INTERVAL_MS;

  while (totalWritten < totalBytesMax) {
    size_t br = 0;
    esp_err_t err = i2s_channel_read(rx_handle, bufPtr, bufBytes, &br, 200);
    if (err != ESP_OK) {
      // ここまでを最大限生かす
      f.flush();
      writeWavHeader(f, s.sampleRate, s.bitsPerSamp, s.channels);  // 現在サイズで確定:contentReference[oaicite:4]{index=4}
      f.flush();
      f.close();
      return RecResult::I2sReadError;
    }
    ...
    // 書込み：セグメント残量と全体残量でクリップ
    const uint32_t segRemain = segBytesTarget - segWritten;
    const uint64_t totalRemain = totalBytesMax - totalWritten;
    const size_t to_write = (size_t)std::min<uint64_t>(avail, std::min<uint64_t>(segRemain, totalRemain));
    ...
    if (to_write > 0) {
      ...
      segWritten += (uint32_t)w;
      totalWritten += (uint64_t)w;
    }

    // 定期“上書き”とflush（ここで絶対にcloseしない）
    uint32_t now = millis();
    if ((int32_t)(now - nextFlushAt) >= 0) {
      f.flush();
      nextFlushAt = now + FLUSH_INTERVAL_MS;
    }
    if ((int32_t)(now - nextHdrAt) >= 0) {
      writeWavHeader(f, s.sampleRate, s.bitsPerSamp, s.channels);  // 上書きのみ:contentReference[oaicite:5]{index=5}
      f.flush();
      nextHdrAt = now + HDR_INTERVAL_MS;
    }

    // ★唯一の切替条件：セグメント「サイズ」満了時のみ
    if (segWritten >= segBytesTarget) {
      writeWavHeader(f, s.sampleRate, s.bitsPerSamp, s.channels);
      f.flush();
      f.close();

      // 次ファイルを開く（失敗してもここまでのファイルは完全）
      if (!open_new_file(f, curPath)) return RecResult::FileOpenError;

      segWritten = 0;
      // セグメント切替では dropHead は二度とかけない
      // DC/AGCの状態も維持（不連続回避）
      nextFlushAt = millis() + FLUSH_INTERVAL_MS;
      nextHdrAt = millis() + HDR_INTERVAL_MS;
    }
  }
